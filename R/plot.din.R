...
#	highest: a numeric between 0 and 1 giving the percentage of skill patterns 
#          with highest occurrence frequency to be printed.
...
################################################################################
	...
	suc <- which(apply(x$item.patt.split, 1, function(y) paste(y,collapse=""))== pattern) #subject under control
	if(pdf.file!="") try(pdf(file=pdf.file, ...))
	  
	try(if(uncertainty<0||uncertainty>.5|| #uncertainty >= 0, <= .5
		highest<0||highest>1)  #highest >= 0, <=1
...
if(1 %in% display.nr){
	...
	if(!hide.idi){
    if(any(apply(errors, 2, function(x) 1-x[1]/(1-x[2]))<0)){
		  warning(paste("Item discrimination index < 0 for item",which(apply(errors, 2, 
		  function(x) 1-x[1]/(1-x[2]))<0),"\n"))
    }else{
	    lines(seq(2,2+3*(ncol(errors)-1),3),apply(errors, 2, function(x) 1-x[1]/(1-x[2])), lty=1)
		  points(seq(2,2+3*(ncol(errors)-1),3),apply(errors, 2, function(x) 1-x[1]/(1-x[2])), pch=19, cex=1.5)
		  legend("topright",c("guessing parameter","slipping parameter", "diagnostic accuracy"), 
...
if(2 %in% display.nr){
	...
	names(skill.patterns) <- colnames(x$q.matrix)[length(x$skill.patt):1]
	master <- (colSums(apply(ifelse(x$pattern[,6:(6+nrow(x$skill.patt)-1)]>0.5+uncertainty,1,NA),
		2, function(y) y*x$pattern[,1]),na.rm=T)/nrow(x$data))[length(x$skill.patt):1]
	...
	text(names(skill.patterns), x=c(rep(0.01,length(skill.patterns))), 
		y=seq(0.7,0.7+1.2*(length(skill.patterns)-1),1.2),col="black", pos=4, cex=1.5)
  ...
  if(!hide.obs){
    ...
    legend("topright",c("Estimated","Observed"), lty=c(1,1), pch=c(NA,19), 
      lwd=c(2,2), col=c("gray", "black"), bg = "gray97")
  }
...
if(3 %in% display.nr){
	...
	plot(c(0:(length(patt.fq)+1)),c(0,t(patt.fq),0),type="h", ylab="Skill pattern occurrence probability",
...
if(4 %in% display.nr){
	# extract information
	post.skill <- rbind(x$pattern[,6:(6+nrow(x$skill.patt)-1)],rep(0,nrow(x$skill.patt)),rep(1,nrow(x$skill.patt)))
	...
	axis(1, at = 1L:ncol(a), labels = colnames(a))
...
if(5 %in% display.nr){   
    ...
    if(length(suc) == 0)
      warning("The specified pattern was not achieved.")
		...
		post.skill <- as.matrix(x$pattern[suc, 6:(6+nrow(x$skill.patt)-1)])[nrow(x$skill.patt):1]
